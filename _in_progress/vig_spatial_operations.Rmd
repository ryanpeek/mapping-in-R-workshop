...
## Join Magical Creatures!

Let's take a look at this globally!
...
myths <- readr::read_csv("data/list_of_mythical_creatures.csv")

set.seed(777)
bb_world = st_bbox(world)
random_df = tibble::tibble(
  x = runif(n = 71, min = bb_world[1], max = bb_world[3]),
  y = runif(n = 71, min = bb_world[2], max = bb_world[4]),
  id=seq(from = 1, to=71, by = 1)
)

myths_sf <- bind_cols(myths, random_df) %>% 
  st_as_sf(coords = c("x", "y"), remove = F) %>% 
  st_set_crs(4326)

mapview::mapview(myths_sf)
