...
title("GSOD Climate Stations Not Contained \n in CA Counties (squares),
      and Counties with no Stations (green)")
```
...
## Play With Some Climate Data

Hopefully the above was useful...but let's actually use grab some climate data from the `GSODR` package and do some visualization. Let's take a look at what stations have data between 1980 and 2018. Check the [`GSODR`](https://ropensci.github.io/GSODR/articles/GSODR.html) vignette for more details, I'm just applying some of the commands they lay describe.

For this example, let's select data from Fresno. I've already downloaded it ([here]()) if you'd like to play along. I'm going to plot the weekly average temperature and precipitation for 1972-2018.

```{r checkStations, echo=FALSE, eval=FALSE, purl=FALSE, warning=FALSE}

# check availability for a date range
loop_stations <- isd_ca_co_pts[isd_ca_co_pts$BEGIN <= 19800101 &
                               isd_ca_co_pts$END >= 20181231, ]

# Plot where these stations are?
ggplot() +
  geom_sf(data=ca, fill="slateblue", alpha=0.4) +
  geom_sf(data=ca_co, color="gray50", lwd=0.4, alpha=0.4) +
  geom_sf(data = loop_stations, size = 2, pch=21, color="gray", fill="black") +
  theme_bw()+
