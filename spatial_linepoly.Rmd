...
I've downloaded rivers for CA and OR and put them [here](https://github.com/ryanpeek/mapping-in-R-workshop/blob/master/data/rivs_CA_OR_hydroshed.zip) on github. Download the zipped file and unzip it in a `data` folder. We're going to use *`shapefiles`* for the remainder of this example. 
...
First let's show a few examples comparing how to plot the rivers dataset using `sf`.

```{r sf_base, echo=T, eval=T}

plot(st_geometry(rivers_sf), col="darkblue", 
     main="Rivers of CA and OR", axes=TRUE)

# this is same
# plot(rivers_sf$geometry), col="darkblue", main="Rivers of CA and OR")
...
# ggsave(filename = "img/ggmap_spatial_linepoly_example.png", width = 4.5, height = 5, units = "in", dpi = 200)

include_graphics("img/ggmap_spatial_linepoly_example.png")
