...
```{r readHidden2, echo=FALSE, eval=TRUE, hide=TRUE}
...
Measuring the same distance between our most upstream USGS gage (Happy Isles) and the most downstream gage (San Joaquin at Jersey Point), I got a distance of **190.7** km. So all in all, I'd say we are doing well. The difference is likely due to slight differences in the projection of the mapview map as well as any innaccuracies associated with where we clicked on the points on the map. Let's move on and try to do this using the actual river distance (along the flowlines).
...
```{r addDistances, echo=TRUE, eval=TRUE}
